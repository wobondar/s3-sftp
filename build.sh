...
do
    ...
    output_name=$bin_name'_'$VERSION'_'$GOOS'_'$GOARCH
    if [ $GOOS = "windows" ]; then
        output_name+='.exe'
    fi

    env GOOS=$GOOS GOARCH=$GOARCH CGO_ENABLED=0 go build -ldflags "${LDFLAGS}" -o ./build/$output_name cmd/$bin_name/*.go
    if [ $? -ne 0 ]; then
...
do
    ...
    GOOS=${platform_split[0]}
    GOARCH=${platform_split[1]}
    input_name=$bin_name'_'$VERSION'_'$GOOS'_'$GOARCH
...
shasum -a 256 *.zip > $bin_name'_'$VERSION'_SHA256SUMS'
echo "Verifying SHA256SUMS"
shasum -a 256 -c $bin_name'_'$VERSION'_SHA256SUMS'
cd ..